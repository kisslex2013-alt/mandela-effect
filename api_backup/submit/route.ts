...
export async function POST(request: NextRequest) {
  try {
    ...
    // Проверяем rate limit (только для реальных IP, но не для localhost)
    ...
    // Создаём объект эффекта для pending-effects.json
    // Преобразуем variantA и variantB из строк в объекты (для совместимости с админкой)
    const newEffect: any = {
      id: `pending_${Date.now()}`, // Временный ID
      ...
      variantA: {
        text: body.variantA.trim(),
        description: body.variantADescription?.trim() || body.currentState?.trim() || '',
      },
      variantB: {
        text: body.variantB.trim(),
        description: body.variantBDescription?.trim() || '',
      },
      ...
      votesA: 0,
      votesB: 0,
      dateAdded: new Date().toISOString().split('T')[0], // Формат YYYY-MM-DD
    };

    // Добавляем интерпретации, если они есть в body
    if ((body as any).interpretations) {
      const interpretations = (body as any).interpretations;
      // Проверяем, что хотя бы одно поле заполнено
      if (interpretations.scientific || interpretations.community) {
        newEffect.interpretations = {
          scientific: interpretations.scientific || '',
          scientificTheory: interpretations.scientificTheory || undefined,
          scientificSource: interpretations.scientificSource || undefined,
          community: interpretations.community || '',
          communitySource: interpretations.communitySource || undefined,
        };
        // Удаляем undefined поля
        if (!newEffect.interpretations.scientificTheory) delete newEffect.interpretations.scientificTheory;
        if (!newEffect.interpretations.scientificSource) delete newEffect.interpretations.scientificSource;
        if (!newEffect.interpretations.communitySource) delete newEffect.interpretations.communitySource;
      }
    }

    // Путь к файлу pending-effects.json
    const pendingPath = path.join(process.cwd(), 'data', 'pending-effects.json');

    // Читаем существующие pending эффекты
    ...
    try {
      ...
      console.log('Файл pending-effects.json не найден, создаём новый');
      pendingEffects = [];
    }

    // Добавляем новый эффект в начало массива
    pendingEffects.unshift(newEffect);
